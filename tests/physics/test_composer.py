...
class TestEndToEnd:
    ...
    def test_all_domains_have_templates(self):
        """Verify that fallback templates exist for pre-1905 classical domains.
        
        Post-1905 domains (quantum, relativistic) must be generated by
        trained models, not hardcoded — they are intentionally absent from
        DOMAIN_TEMPLATES.
        """
        pre1905_domains = {"gravity", "spring", "em", "thermal", "collision"}
        for domain in pre1905_domains:
        ...
        # Post-1905 domains should NOT have hardcoded templates
        assert "quantum" not in DOMAIN_TEMPLATES, (
            "Quantum template must be generated by trained model, not hardcoded"
        )
        assert "relativistic" not in DOMAIN_TEMPLATES, (
            "Relativistic template must be generated by trained model, not hardcoded"
        )

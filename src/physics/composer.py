...
# NOTE: Only pre-1905 classical domains. Post-1905 (quantum, relativistic) 
# must be generated by trained models, not hardcoded.
...
class PerDomainComposer(nn.Module):
    ...
    def forward(
        ...
        for domain in active_domains:
            ...
            # Fallback: use hardcoded template when generator output is empty
            # or when generator is not trained (fresh random weights)
            if not tmpl and domain in DOMAIN_TEMPLATES:
...
def load_self_play_generators(
    checkpoint_dir: str | Path,
    device: torch.device = torch.device("cpu"),
    domains: list[str] | None = None,
) -> dict[str, DomainTemplateGenerator]:
    """Load self-play trained template generators.

    Loads per-domain checkpoints trained on self-play data (Phase D).
    These generators are trained on structural patterns from pre-1905
    quantities and generalize to post-1905 symbols.

    Args:
        checkpoint_dir: Directory containing self_play_<domain>_template.pt files.
        device: Device to load models onto.
        domains: Domains to load (default: all with checkpoints).

    Returns:
        Dict mapping domain name to trained DomainTemplateGenerator.
    """
    checkpoint_dir = Path(checkpoint_dir)
    if domains is None:
        # Auto-detect: any file matching self_play_*_template.pt
        domains = []
        for p in checkpoint_dir.glob("self_play_*_template.pt"):
            name = p.stem
            if name.startswith("self_play_") and name.endswith("_template"):
                domain = name[len("self_play_"):-len("_template")]
    ...
    for domain in domains:
        ...
        if ckpt_path.exists():
            gen = load_domain_generator(str(ckpt_path))
            gen = gen.to(device)
            gen.eval()
            generators[domain] = gen
...
def load_self_play_composer(
    checkpoint_dir: str | Path,
    device: torch.device = torch.device("cpu"),
    classifier_path: str | Path | None = None,
) -> PerDomainComposer:
    """Load a full PerDomainComposer with self-play trained generators.

    Loads domain classifier (trained or fresh) and self-play template
    generators. Uses DOMAIN_TEMPLATES as fallback for domains without
    trained generators.

    Args:
        checkpoint_dir: Directory containing self_play checkpoints.
        device: Device to load models onto.
        classifier_path: Optional path to domain_classifier.pt.

    Returns:
        PerDomainComposer with self-play generators.
    """
    checkpoint_dir = Path(checkpoint_dir)

    # Load domain classifier
    clf_path = (
        Path(classifier_path) if classifier_path
        else checkpoint_dir / "domain_classifier.pt"
    )
    if clf_path.exists():
        try:
            clf = load_domain_classifier(str(clf_path))
        except Exception:
            clf = DomainClassifier()
    else:
        clf = DomainClassifier()
    ...
    # Load self-play generators
    generators = load_self_play_generators(checkpoint_dir, device)

    # Fill missing domains with fresh (untrained) generators
    # They'll fall back to DOMAIN_TEMPLATES at inference time
    for domain in DOMAINS:
    ...
    # Load collision generator if available
    col_path = checkpoint_dir / "self_play_collision_template.pt"
    if col_path.exists():
        col_gen = load_domain_generator(str(col_path))
    else:
        col_gen = CollisionTemplateGenerator()
    col_gen = col_gen.to(device)
    col_gen.eval()
    generators[COLLISION_DOMAIN] = col_gen

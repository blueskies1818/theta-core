...
12 CPU threads. Output: data/gate3_v2_full_result.json

Usage:
    python scripts/eval/run_full_gate3_v2.py [--max-expansions N] [--top-k N]
"""
...
def run_gate3_full(
    ...
    for i, t in enumerate(theorems):
        ...
        proof_steps, final_state = bf_search.search(stmt, domain=search_domain, verbose=False)
    ...
    # --- Build output ---
    out = {
        "task": "FULL gate3_v2 benchmark: Hybrid (GNN + Best-First + Dense Rewards)",
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        "architecture": "GNN cosine similarity (MRR 0.786) + Best-first search",
        "domain_filtering": {
            "enabled": use_domain_filter,
            "description": "Filter lemma candidates to same Mathlib domain as goal theorem before GNN scoring",
        },
        "config": {
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run hybrid architecture on FULL gate3_v2 benchmark (64 theorems)"
    )
    parser.add_argument(
        "--gnn-checkpoint",
    ...
    parser.add_argument(
        "--output", default="data/gate3_v2_domain_filtered.json",
    ...
    print("=" * 70)
    print("FULL gate3_v2 BENCHMARK: Hybrid Architecture Run")
    print("=" * 70)

#!/usr/bin/env python3
"""Extended ERA GATE v2 — evaluation with HiddenVariableProposer v2.

Tests all 10 scenarios (8 post-1905 + 2 classical) using the v2 model
with continuous ratio, additive offset, and multi-variable group proposals.

Key improvements over v1:
  - Photoelectric: h as quantity, phi hidden (multi-variable TYPE 3)
  - Relativistic: correct target dimensions, deeper search
  - All scenarios: uses v2 model with 7 variable types

RUN: python scripts/extended_era_gate_v2.py
OUTPUTS:
  data/extended_era_gate_v2_results.json
  docs/reports/extended_era_gate_v2.md
"""
...
RESULTS_PATH = PROJECT_ROOT / "data" / "era_gate_depth10_results.json"
REPORT_PATH = PROJECT_ROOT / "docs" / "reports" / "extended_era_gate_v3.md"
...
class ExtendedResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "proposal": {"type": self.top_proposal_type, "transform": self.top_proposal_transform, "confidence": self.top_proposal_confidence, "num_tried": self.num_proposals_tried, "all": self.proposals},
...
def make_photoelectric_obs() -> list[Observation]:
    """v2: h is a quantity, phi is hidden. Tests multi-variable discovery.

    Baseline: {K_max, f} — no constant expression discoverable
    After proposing phi: h*f - K_max = phi = constant
    """
...
def make_simple_relativistic_obs() -> list[Observation]:
    ...
    TYPE 1 continuous ratio: errors scale with E → propose gamma.
    After proposing gamma: E/gamma = E0 = constant.
    """
    E0 = 938.0  # MeV (rest energy)
    timesteps = []
...
def _is_trivial_discovery(expr, score, observations=None):
    ...
    if re.search(r'([A-Za-z_]\w*)-(\1)(?!\w)', e):
        return True
    # v3: catch self-division (tau/tau, x/x, etc.) and trivial identity patterns
    if re.search(r'^([A-Za-z_]\w*)/(\1)$', e):
        return True
    if re.search(r'^([A-Za-z_]\w*)\*0$', e):
        return True
    if re.search(r'^0\*[A-Za-z_]\w*$', e):
        return True
    return False
...
def test_scenario(scenario_id, scenario_name, domain, category, known_invariant,
    ...
    try:
        ...
        result.proposals = [{"type": p.variable_type, "name": p.variable_name,
            ...
            "expression_fragment": getattr(p, 'expression_fragment', '')}
...
def main():
    ...
    print("=" * 70)
    print("Extended ERA GATE v2 — 8 Post-1905 + 2 Classical Tests")
    print("=" * 70)

    print("\n[1] Loading HiddenVariableProposer v2...")
    if CHECKPOINT_PATH.exists():
        proposer = load_hidden_var_proposer(str(CHECKPOINT_PATH))
        print(f"    Loaded v2 checkpoint ({proposer.count_parameters()} params)")
    else:
        proposer = HiddenVariableProposer()
        print(f"    Fresh v2 model ({proposer.count_parameters()} params)")
    ...
    scenario_specs = [
        ...
        {"scenario_id": "time_dilation", "scenario_name": "Time dilation delta_t = gamma*delta_tau",
         ...
         "target_dim": "Scalar", "max_depth": 12},
    ...
    output = {"experiment": "extended_era_gate_v2", "timestamp": time.time(),
    ...
    for r in results:
        ...
        print(f"  {status} {r.scenario_id:<28s} {r.best_expression[:35]:<35s} score={r.best_score:.4f}")

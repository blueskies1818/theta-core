...
Usage:
    ...
    python scripts/training/train_explorer.py --domain GroupTheory --max-theorems 200 --steps 50

    # Train on Algebra (16,800 nodes)
    python scripts/training/train_explorer.py --domain Algebra --max-theorems 500 --steps 100

    # Train on full graph (58K nodes — heavy)
    python scripts/training/train_explorer.py --full-graph --max-theorems 500 --steps 20

    # Resume from checkpoint
    python scripts/training/train_explorer.py --domain Algebra --resume checkpoints/explorer/gnn_step_50.pt
"""
...
def build_graph(
    ...
    if not full_path.with_suffix(".nx.pkl").exists():
        raise FileNotFoundError(
            ...
            f"Run: python scripts/build/build_dependency_graph.py"
        )
...
Examples:
  ...
  %(prog)s --full-graph --max-theorems 1000 --steps 20 --device cuda:0
    ...
    if not jsonl_path.exists():
        ...
        print("Run: python scripts/build/prepare_data.py")
        sys.exit(1)
